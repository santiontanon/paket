...
public class CIEDE2000 {
    ...
    public static int[] convertPixelBlock(int pixels[], int y)
    {
        int octetr[] = new int[8];
        int octetg[] = new int[8];
        int octetb[] = new int[8];
        ...
        for (int i = 0; i < 8; i++) {
            // Get the RGB values of 8 pixels of the original image
            octetr[i] = MSXColors.MSX1Palette[pixels[i]][0];
            octetg[i] = MSXColors.MSX1Palette[pixels[i]][1];
            octetb[i] = MSXColors.MSX1Palette[pixels[i]][2];
        }
        ...
        for (int cor1 = 1; cor1 <= N_COLORS; cor1++) {
            for (int cor2 = cor1; cor2 <= N_COLORS; cor2++) {
                ...
                // If KeyHit(1) Then End
                // First MSX color of the octet
                toner[0] = MSXColors.MSX1Palette[cor1][0];
                toneg[0] = MSXColors.MSX1Palette[cor1][1];
                toneb[0] = MSXColors.MSX1Palette[cor1][2];

                // A mix of both MSX colors RGB values. Since MSX cannot mix colors,
                // later if this color is chosen it'll be substituted by a 2x2 dithering pattern.
                toner[1] = (MSXColors.MSX1Palette[cor1][0] + MSXColors.MSX1Palette[cor2][0]) / 2;
                toneg[1] = (MSXColors.MSX1Palette[cor1][1] + MSXColors.MSX1Palette[cor2][1]) / 2;
                toneb[1] = (MSXColors.MSX1Palette[cor1][2] + MSXColors.MSX1Palette[cor2][2]) / 2;

                // Second MSX color of the octet
                toner[2] = MSXColors.MSX1Palette[cor2][0];
                toneg[2] = MSXColors.MSX1Palette[cor2][1];
                toneb[2] = MSXColors.MSX1Palette[cor2][2];
                ...
                if (DITHERING
                    ...
                    double dist = 0;
                    for (int i = 0; i <= 7; i++) {
                        double finaldista = calcdist2000(toner[0], toneg[0], toneb[0], octetr[i], octetg[i], octetb[i]);
                        double finaldistb = calcdist2000(toner[2], toneg[2], toneb[2], octetr[i], octetg[i], octetb[i]);
                        double finaldist;
        ...
        int outputPixels[] = new int[8];
        for (int i = 0; i < 8; i++) {
            switch (octetfinal[i]) {
                case 0:
                    outputPixels[i] = bestcor1;
                    break;
                case 1:
                    if (y % 2 == i % 2) {
                        outputPixels[i] = bestcor2;
                    } else {
                        outputPixels[i] = bestcor1;
                    }
                    break;
